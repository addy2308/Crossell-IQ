...
def build_features():
    ...
    # Load raw synthetic data (we'll switch to real data in a moment)
    ...
    if 'cross_sell_flag' in df.columns:
        ...
        df['target'] = ((df['had_claim_60d']*0.3 + df['renewal_in_30_days']*0.3 + df['owns_B']*0.2 + (df['income']>500000).astype(int)*0.2) > 0.4).astype(int)
